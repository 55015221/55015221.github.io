      ...
      <ul @mouseleave="onColumnsAsideMenuMouseleave()">
        ...
        >
          <div :class="themeConfig.columnsAsideLayout" v-if="!v.meta.isLink || (v.meta.isLink && v.meta.isIframe)">
            <Icon :size="20" v-if="v.meta.icon" :name="v.meta.icon" />
            <div class="columns-vertical-title font13">
...
const { routeList, isColumnsMenuHover, isColumnsNavHover } = storeToRefs(stores);
...
const setColumnsAsideMove = (k: number) => {
  ...
  state.liIndex = k;
  columnsAsideActiveRef.value.style.top = `${columnsAsideOffsetTopRefs.value[k].offsetTop + 3 + state.difference}px`;
};
// 菜单高亮点击事件
...
const onColumnsAsideMenuMouseleave = async () => {
  ...
  // 添加延时器，防止拿到的 store.state.routeList 值不是最新的
  setTimeout(() => {
...
const setFilterRoutes = () => {
  state.columnsAsideList = filterRoutesFun(routeList.value);
