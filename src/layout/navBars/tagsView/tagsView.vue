    ...
    <el-scrollbar ref="scrollbarRef" @wheel.prevent="onHandleScroll">
      <ul class="layout-navbars-tagsview-ul" :class="setTagsStyle" ref="tagsUlRef">
        ...
        >
          ...
          <Icon :name="v.meta.icon" v-if="!isActive(v) && getThemeConfig.isTagsviewIcon" class="pr5" />
          <span>{{ setTagsViewNameI18n(v) }}</span>
          <template v-if="isActive(v)">
            <Icon name="RefreshRight" class="ml5 layout-navbars-tagsview-ul-li-refresh" @click.stop="refreshCurrentTagsView($route.fullPath)" />
            <Icon
              name="Close"
          ...
          <Icon name="Close" class="layout-navbars-tagsview-ul-li-icon layout-icon-three" v-if="!v.meta.isAffix" @click.stop="closeCurrentTagsView(getThemeConfig.isShareTagsView ? v.path : v.url)" />
        </li>
      </ul>
    </el-scrollbar>
...
const storesRoutesList = useRouteListStore();
  ...
  if (Session.get("tagsViewList") && getThemeConfig.value.isCacheTagsView) {
    ...
    await state.tagsViewRoutesList.map((v: RouteItem) => {
      if (v.meta?.isAffix && !v.meta.isHidden) {
        ...
        keepAliveStore.addCachedView(v);
      }
    });
...
const solveAddTagsView = async (path: string, to?: RouteToFrom) => {
  ...
  if (current.length <= 0) {
    ...
    await keepAliveStore.addCachedView(findItem);
...
const addTagsView = (path: string, to?: RouteToFrom) => {
  ...
  nextTick(async () => {
    ...
    await keepAliveStore.addCachedView(item);
...
const refreshCurrentTagsView = async (fullPath: string) => {
  ...
  if (!item) return false;
  await keepAliveStore.delCachedView(item);
  mittBus.emit("onTagsViewRefreshRouterView", fullPath);
  if (item.meta?.isKeepAlive) await keepAliveStore.addCachedView(item);
};
  ...
  state.tagsViewList.map((v: RouteItem, k: number, arr: RouteItems) => {
    if (!v.meta?.isAffix) {
      if (getThemeConfig.value.isShareTagsView ? v.path === path : v.url === path) {
        keepAliveStore.delCachedView(v);
...
const closeOtherTagsView = (path: string) => {
  if (Session.get("tagsViewList")) {
    ...
    Session.get("tagsViewList").map((v: RouteItem) => {
      if (v.meta?.isAffix && !v.meta.isHidden) {
        v.url = setTagsViewHighlight(v);
        keepAliveStore.delOthersCachedViews(v);
        state.tagsViewList.push({ ...v });
      }
    });
...
const closeAllTagsView = () => {
  if (Session.get("tagsViewList")) {
    keepAliveStore.delAllCachedViews();
    state.tagsViewList = [];
    Session.get("tagsViewList").map((v: RouteItem) => {
      if (v.meta?.isAffix && !v.meta.isHidden) {
        v.url = setTagsViewHighlight(v);
...
const onTagsClick = (v: RouteItem, k: number) => {
  ...
  if (getThemeConfig.value.layout === "columns") {
    const item: RouteItem = routeList.value.find((r: RouteItem) => r.path.indexOf(`/${v.path.split("/")[1]}`) > -1);
...
onBeforeMount(() => {
  ...
  mittBus.on("onCurrentContextmenuClick" as any, (data: RouteItem) => {
    // 通过方法点击关闭 tagsView
    data.isFnClick = true;
    onCurrentContextmenuClick(data);
  });
  // 监听布局配置界面开启/关闭拖拽
  mittBus.on("openOrCloseSortable" as any, () => {
    initSortable();
  });
  // 监听布局配置开启 TagsView 共用，为了演示还原默认值
  mittBus.on("openShareTagsView" as any, () => {
    if (getThemeConfig.value.isShareTagsView) {
      router.push("/");
      state.tagsViewList = [];
