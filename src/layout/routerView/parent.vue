...
const storesKeepAliveNames = useKeepAliveStore();
const themeStore = useThemeStore();
const { keepAlive, cachedViews } = storeToRefs(storesKeepAliveNames);
...
onBeforeMount(() => {
  state.keepAliveNameList = keepAlive.value;
  mittBus.on("onTagsViewRefreshRouterView", (fullPath: string) => {
    state.keepAliveNameList = keepAlive.value.filter((name: string) => route.name !== name);
  ...
  nextTick(() => {
    setTimeout(() => {
      if (themeConfig.value.isCacheTagsView) {
        let tagsViewArr: RouteItem[] = Session.get("tagViewList") || [];
