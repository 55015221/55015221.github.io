...
import { ElMessage, ElNotification, genFileId, UploadFile, UploadFiles, UploadInstance, UploadProgressEvent, UploadProps, UploadRawFile, UploadRequestOptions, UploadUserFile } from "element-plus";
...
interface UploadFileProps {
  name?: string;
  modelValue?: FileInterface[]; // 图片 可以是数组也可以是字符串
...
const limiter = computed(() => {
  return props.limit - fileList.length;
});
...
//当超出限制时，执行的钩子函数
const onExceed: UploadProps["onExceed"] = (files: File[], uploadFiles: UploadUserFile[]) => {
  console.log(files);
  console.log(uploadFiles);
  if (fileList.length + files.length > props.limit) {
...
// 上传进度
const onProgress = (event: UploadProgressEvent, file: UploadFile, files: UploadFiles) => (progress.percent = ~~event.percent);
...
const httpRequest = async (options: UploadRequestOptions) => {
  const { file, onSuccess, onError } = options; // 获取传入的 files（即所有选择的文件）

  // 创建 FormData 对象并添加文件
  const formData = new FormData();
  formData.append("file", file); // 这里使用 'files' 为表单字段名
  ...
  <div class="upload-container">
    ...
    >
      ...
      <div v-show="progress.percent" class="progress" :style="`width:${width}px;height:${height}px;`">
        <el-image :src="progress.preview" :style="`width:${width}px;height:${height}px;`" fit="cover" />
    ...
    <div class="el-upload__tip">
      <slot name="tip">
        <el-alert v-if="!$slots.tip" :title="`上传图片支持 ${accept} 格式,单张图片大小不超过 ${size}MB,且图片数量不超过 ${limit} 张`" type="warning" show-icon :closable="false" />
      </slot>
    </div>
