<template>
  <div class="wrapper">
    <vxe-grid ref="gridRef" v-bind="gridOptions">
      ...
      <template #slotPermissions="{ row }">
        <el-space wrap>
          <el-tag v-for="(value, _) in row.permissions" :key="value.id">{{ value }}</el-tag>
        </el-space>
      </template>

      <template #avatar="{ row }">
        <ImagePreview :src="row.avatar" width="80" height="80" :hide-on-click-modal="true"></ImagePreview>
      </template>
    </vxe-grid>
...
import { VxeGridInstance, VxeGridProps, VxeTableInstance } from "vxe-table";
import VxeUI from "vxe-pc-ui";
import ApiService from "@/api/admin/role";
...
const gridOptions = reactive<VxeGridProps<RowVO>>({
  ...
  columns: [
    ...
    { title: "权限集", slots: { default: "slotPermissions" } },
