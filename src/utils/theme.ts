...
export function useChangeColor() {
  ...
  const hexToRgb = (str: string): any => {
    ...
    let hexs: any = "";
    let reg = /^\#?[0-9A-Fa-f]{6}$/;
    let rgba = /rgba\(\d+,\d+,\d+,(0|1|0?\.\d+)\)/;
    if (!(reg.test(str) || rgba.test(str.replace(/\s+/g, "")))) {
      console.error(str);
  ...
  const rgbToHex = (r: any, g: any, b: any): string => {
    ...
    if (!reg.test(r) || !reg.test(g) || !reg.test(b)) {
      ElMessage.warning("输入错误的rgb颜色值1" + r);
      return "";
    }
  ...
  const getDarkColor = (color: string, level: number): string => {
    let reg = /^\#?[0-9A-Fa-f]{6}$/;
    let rgba = /rgba\(\d+,\d+,\d+,(0|1|0?\.\d+)\)/;
    if (!(reg.test(color) || rgba.test(color.replace(/\s+/g, "")))) {
      console.error("1 输入错误的hex颜色值" + color);
      ElMessage.warning("输入错误的hex颜色值" + color);
      return "";
    }
  ...
  const getLightColor = (color: string, level: number): string => {
    ...
    let rgba = /rgba\(\d+,\d+,\d+,(0|1|0?\.\d+)\)/;
    if (!(reg.test(color) || rgba.test(color.replace(/\s+/g, "")))) {
      console.error("2 输入错误的hex颜色值" + color);
      ElMessage.warning("输入错误的hex颜色值" + color);
      return "";
    }
    let rgb = useChangeColor().hexToRgb(color);
